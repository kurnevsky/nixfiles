...
let
          ...
          pkgs.symlinkJoin {
            ...
            postBuild = ''
              ...
              grep -RlP "\\Q${
                ...
                if ${lib.concatMapStringsSep " || " (bin: ''[[ "$file" == "bin/${bin.name}" ]]'') bins}; then
                  continue
                fi
                rm -f "$out/$file"
  ...
  withFonts =
    ...
    attrs
    // {
      extra-deps = (attrs.extra-deps or [ ]) ++ config.fonts.fontconfig.confPackages;
      etcs = (attrs.etcs or [ ]) ++ [ "fonts" ];
      whitelist =
        if builtins.elem "~/" attrs.whitelist or [ ] then
          attrs.whitelist
        else
...
in
{
    ...
    (self: _super: {
      sandboxed = {
        ...
        zathura = wrap self.zathura [
          (
            let
              cfg = viewer-cfg "zathura";
            in
            cfg
            // {
              extra-deps = with pkgs; cfg.extra-deps ++ [ config.i18n.glibcLocales ];
        ...
        kdePackages.okular = wrap self.kdePackages.okular [
          (
            let
              cfg = withOpengl (viewer-cfg "okular");
            in
            cfg
            // {
              extra-deps =
  ...
  environment = {
    ...
    etc =
      let
        home-files = lib.mapAttrsToList (_name: value: value.home-files) config.home-manager.users;
        home-paths = lib.mapAttrsToList (_name: value: value.home.path) config.home-manager.users;
        home-deps-drv =
          paths:
          let
            drv = pkgs.runCommand "home-files" { disallowedReferences = home-files; } (
